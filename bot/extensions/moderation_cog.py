...
class ModerationCog(
    ...
    async def on_reaction_add(self, reaction: Reaction, member: Member) -> None:
        ...
        match demojize(str(reaction.emoji)):
            ...
            case ":red_question_mark:":
                guidelines: Channel = Channel.find_by(channel_name="posting_guidelines")
                help: Channel = Channel.find_by(channel_name="help")
    ...
    async def on_member_join(self, member) -> None:
        ...
        if account_age_in_days < minimum_account_age:
            ...
            await member.send(
                f"Your account needs to be {minimum_account_age} days old or more to join the server."
            )
