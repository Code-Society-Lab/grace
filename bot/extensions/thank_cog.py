...
class ThankCog(Cog):
    ...
    async def thank(self, ctx: Context, *, member: Member) -> Optional[Message]:
        ...
        if member.id == self.bot.user.id:
            return await ctx.send(
                f'{ctx.author.display_name}, thank you 😊', ephemeral=True
            )
        ...
        thank_embed: Embed = Embed(
            ...
            description=f'{member.display_name}, you were thanked by'
        ...
        await ctx.interaction.response.send_message(
            f'Successfully thanked **@{member.display_name}**', ephemeral=True
        )

    @thank_group.command(name='leaderboard', description='Shows top n helpers.')
    async def thank_leaderboard(
        self, ctx: Context, *, top: int = 10
    ) -> Optional[Message]:
        ...
        if top <= 0:
            return await ctx.reply(
                'The top parameter must have value of at least 1.', ephemeral=True
            )
        ...
        for position in range(top):
            ...
            leaderboard_embed.description += (
                '{}. **{}**: **{}** with {} thank(s).\n'.format(
                    position + 1, member_nickname, member.rank, member.count
                )
            )
    ...
    async def send_bot_rank(self, ctx: Context) -> None:
        ...
        """
        bot_rank_msg = (
            'Grace has a range of commands that can help you greatly!\nRank: **Bot**'
        )

        rank_embed: Embed = Embed(title='Grace RANK', color=self.bot.default_color)
        rank_embed.description = bot_rank_msg
    ...
    async def send_member_rank(self, ctx: Context, member: Member) -> None:
        ...
        :type member: Member
        """
        rank_embed: Embed = Embed(
            title=f'{member.display_name} RANK', color=self.bot.default_color
        )
        thank = Thank.get_by(member_id=member.id)

        if not thank:
            rank_embed.description = (
                f"User **@{member.display_name}** hasn'tbeen thanked yet."
            )
        else:
            rank_embed.description = (
                f'User **@{member.display_name}** has rank:**{thank.rank}**'
            )
